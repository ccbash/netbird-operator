...
| Log level | The proxy always embeds a netbird client (`NB_PROXY_WG_PORT`), whose periodic ICE monitor can't gather P2P candidates in-cluster and logs `wait for gathering timed out` every tick. Centralised serving never uses that P2P path (peers hit the proxy's LB IP), so it's pure noise — set `logLevel: error` (params) → `NB_PROXY_LOG_LEVEL`/`NB_LOG_LEVEL` to silence it. A private/embedded cluster instead *needs* ICE to work; raise the level and fix UDP egress / pod networking there. |
