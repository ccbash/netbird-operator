...
| `domain` _string_ | Domain is the public hostname clients connect to. For mode=http/tls it is<br />the service domain verbatim (HTTP routing / TLS SNI). For mode=tcp/udp it<br />is the shared host: NetBird allows only one service per domain, and L4<br />connections route by listen port (no SNI), so the operator publishes each<br />port as a distinct per-port subdomain (<mode>-<listenPort>.<Domain>, shown<br />in status.serviceDomain) under this host. Expose several L4 ports under one<br />hostname with one CR per port, all sharing this Domain. The host must<br />resolve to a proxy cluster (be the cluster address, its subdomain, or a<br />registered NetBird custom domain) and have public DNS pointing at it. |  | MinLength: 1 <br /> |
...
| `serviceDomain` _string_ | ServiceDomain is the domain actually registered with NetBird. It equals<br />spec.domain for http/tls, and the synthesized per-port subdomain<br />(<mode>-<listenPort>.<spec.domain>) for tcp/udp. |  | Optional: \{\} <br /> |
